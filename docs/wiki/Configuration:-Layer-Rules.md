...
- `animate`: set to `true` to make niri continuously track the pointer for this surface and pass its position as `niri_pointer` to any `custom-shader`. Without it, `niri_pointer` is always `(-1, -1)`. Default: `false`. <sup>Since: next release</sup>
- `custom-shader`: path to a `.frag` file (or an inline GLSL raw string) used as a post-processing shader for this surface's background. See [Background Shaders](./Configuration:-Background-Shaders.md) for details. <sup>Since: next release</sup>
