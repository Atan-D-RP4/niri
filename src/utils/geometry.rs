...
/// Compositor-wide frame: cross-output positions and deltas, in logical units.
///
/// Used for hit-testing entry points, cursor/pointer-grab state, and surface origins.
/// Conversion to/from [`Local`] is a pure origin shift through [`OutputViewCtx`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Global;

/// Per-output frame: output-relative positions and deltas, in logical units.
///
/// Used for render locations and tile/layer geometry. Values may persist in layout data;
/// the frame marks scope, not lifetime.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Local;

/// Surface-relative positions and offsets.
///
/// Not a persistent compositor frame: it keeps surface-relative input offsets distinct
/// from global positions. Do not add more frame markers without a call-site-driven reason.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl<C: Coordinate> PointLocalExt<C> for Point<C, Local> {
    ...
    fn to_global(self, ctx: &OutputViewCtx) -> Point<C, Global> {
        // NOTE: No output-transform application since Locals are already in presented
        // orientation (the transform is applied when sizing local_geo), so only
        // the origin shift converts frames.
