...
mod lua_config_tests {
    ...
    use crate::lua_extensions::{apply_lua_config, LuaConfig};
    ...
    fn test_lua_config_with_returned_table() {
        ...
        assert_eq!(
            config.binds.0.len(),
            original_binds + 2,
            "Should have 2 new bindings"
        );

        // Verify startup commands were extracted from returned table
        assert!(
            config.spawn_at_startup.len() >= original_cmds + 2,
            "Should have added startup commands"
        );
