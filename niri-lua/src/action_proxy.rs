...
impl LuaUserData for ActionProxy {
    fn add_methods<M: LuaUserDataMethods<Self>>(methods: &mut M) {
        ...
        // spawn(command, opts?) - command is array of strings
        ...
        // spawn_sh(command, opts?) - command is a shell string
        // When opts is provided, returns ProcessHandle; otherwise fire-and-forget
        methods.add_method(
            "spawn_sh",
...
mod tests {
    ...
    // ============================================================
    // Spawn with Options Tests (ProcessHandle)
    // ============================================================
    ...
    fn test_spawn_with_detach_returns_nil() {
        ...
        // Spawn with detach=true should return nil and dispatch action
        let result: LuaValue = lua
            .load(r#"return niri.action:spawn({"echo", "detached"}, {detach = true})"#)
            .eval()
            .unwrap();

        assert!(
            matches!(result, LuaValue::Nil),
            "Expected Nil for detached spawn, got {:?}",
            result
        );

        // Should have dispatched a Spawn action
        let actions = actions.lock().unwrap();
        assert_eq!(actions.len(), 1);
    ...
    fn test_spawn_sh_with_detach_returns_nil() {
        ...
        // spawn_sh with detach=true should return nil
        let result: LuaValue = lua
            .load(r#"return niri.action:spawn_sh("echo detached", {detach = true})"#)
            .eval()
            .unwrap();

        assert!(matches!(result, LuaValue::Nil));

        // Should have dispatched a SpawnSh action
        let actions = actions.lock().unwrap();
        assert_eq!(actions.len(), 1);
    ...
    #[test]
    fn test_spawn_without_opts_fire_and_forget() {
        let (lua, actions) = create_test_env();

        // Original behavior: spawn without opts should return nil and dispatch action
        let result: LuaValue = lua
            .load(r#"return niri.action:spawn({"echo", "fire-and-forget"})"#)
            .eval()
            .unwrap();

        assert!(
            matches!(result, LuaValue::Nil),
            "Expected Nil for fire-and-forget spawn, got {:?}",
            result
        );
        ...
        assert_eq!(actions.len(), 1);
        if let Action::Spawn { command } = &actions[0] {
            assert_eq!(command, &vec!["echo", "fire-and-forget"]);
        } else {
            panic!("Expected Spawn action");
        }
    }
